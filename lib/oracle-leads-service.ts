  ...
  try {
    ...
    const sql = `
      INSERT INTO AD_ADLEADSATIVIDADES (
        CODLEAD, ID_EMPRESA, TIPO, TITULO, DESCRICAO, DATA_HORA, DATA_INICIO, DATA_FIM,
    ...
    // Buscar atividade criada pela maior ORDEM
    const sqlBuscar = `
      SELECT 
        CODATIVIDADE,
        CODLEAD,
    ...
    const atividadeCriada = await oracleService.executeOne<any>(sqlBuscar, { idEmpresa, ordem: novaOrdem });

    if (!atividadeCriada) {
      throw new Error('Atividade criada mas não foi possível recuperá-la');
    }

    // Converter data do formato Oracle para ISO
    const converterDataOracleParaISO = (dataOracle: string) => {
      if (!dataOracle) return new Date().toISOString();
    ...
    return {
      CODATIVIDADE: atividadeCriada.CODATIVIDADE?.toString() || '',
      CODLEAD: atividadeCriada.CODLEAD?.toString() || '',
      TIPO: atividadeCriada.TIPO || 'TAREFA',
      ...
      CODUSUARIO: atividadeCriada.CODUSUARIO,
      DADOS_COMPLEMENTARES: atividadeCriada.DADOS_COMPLEMENTARES || {},
      NOME_USUARIO: atividadeCriada.NOME_USUARIO || '',
      COR: atividadeCriada.COR || '#22C55E',
      ORDEM: atividadeCriada.ORDEM,
      ATIVO: atividadeCriada.ATIVO || 'S',
      STATUS: atividadeCriada.STATUS || 'AGUARDANDO'
    } as LeadAtividade;
