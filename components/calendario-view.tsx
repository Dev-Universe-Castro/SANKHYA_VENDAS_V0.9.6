...
export default function CalendarioView() {
  ...
  const loadEventos = async () => {
    try {
      ...
      console.log('📊 [Calendário] Dados recebidos da API:', data)
      console.log('📊 [Calendário] Total de eventos:', data.length)

      // Separar ativos e inativos
      const ativos = data.filter((ev: CalendarioEvento) => ev.ATIVO !== 'N')
      const inativos = data.filter((ev: CalendarioEvento) => ev.ATIVO === 'N')

      console.log('✅ [Calendário] Eventos ativos:', ativos.length)
      console.log('❌ [Calendário] Eventos inativos:', inativos.length)
      
      if (ativos.length > 0) {
        console.log('📅 [Calendário] Primeiro evento ativo:', ativos[0])
      }
